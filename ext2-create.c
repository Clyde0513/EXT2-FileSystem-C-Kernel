...
void write_superblock(int fd)
{
	...
	superblock.s_free_blocks_count = NUM_FREE_BLOCKS;	//-1;
	superblock.s_free_inodes_count = NUM_FREE_INODES;	//-1;
	superblock.s_first_data_block = SUPERBLOCK_BLOCKNO; //-1;				  /* First Data Block */
	superblock.s_log_block_size = 0;					// 0;						 /* 1024 */
	superblock.s_log_frag_size = 0;						// 0;							 /* 1024 */
	superblock.s_blocks_per_group = NUM_BLOCKS * 8;		//-1;
...
void write_block_group_descriptor_table(int fd)
{
	...
	block_group_descriptor.bg_block_bitmap = BLOCK_BITMAP_BLOCKNO; //-1;
	block_group_descriptor.bg_inode_bitmap = INODE_BITMAP_BLOCKNO; //-1;
	block_group_descriptor.bg_inode_table = INODE_TABLE_BLOCKNO;   //-1;
	block_group_descriptor.bg_free_blocks_count = NUM_FREE_BLOCKS; //-1;
...
void write_block_bitmap(int fd)
{
	...
	{
		if (i == 0 || i == 1 || i > 127)
		// more than 127, exactly 0, or exactly 1 indices, turn the rest of the block bitmap be 255.
		{
			map_value[i] = 255;
		}
		else if (i == 2)
...
void write_inode_bitmap(int fd)
{
	...
	{
		if (i == 0 || i >= 16)
		// more than or equal to 16, or equal to 0, put the rest of the block bitmap be 255.
		{
			map_value[i] = 255;
		}
		else if (i == 1)
...
void write_inode_table(int fd)
{
	...
	const char hello_world[12] = "hello-world"; // added an extra space for null byte
												// hello -> hello-world (symbolic link)
	// ROOT INODE
	struct ext2_inode inode_root = {0};
	inode_root.i_mode = EXT2_S_IFDIR | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IXUSR | EXT2_S_IRGRP | EXT2_S_IXGRP | EXT2_S_IROTH | EXT2_S_IXOTH;
	inode_root.i_uid = 0;
	...
	inode_root.i_blocks = 2; /* These are oddly 512 blocks */
	inode_root.i_block[0] = ROOT_DIR_BLOCKNO;
	write_inode(fd, EXT2_ROOT_INO, &inode_root); // 2 is EXT2_ROOT_INO

	// HELLO Inode
	struct ext2_inode inode_hello = {0};
	inode_hello.i_mode = EXT2_S_IFLNK | EXT2_S_IRUSR | EXT2_S_IWUSR | EXT2_S_IRGRP | EXT2_S_IROTH;
	inode_hello.i_size = 11;
	inode_hello.i_atime = current_time;
	inode_hello.i_ctime = current_time;
	inode_hello.i_mtime = current_time;
	inode_hello.i_dtime = 0;
	inode_hello.i_gid = 1000;
	inode_hello.i_links_count = 1;
	inode_hello.i_blocks = 0; /* These are oddly 512 blocks */
	memcpy(&(inode_hello.i_block), hello_world, 12);
	write_inode(fd, HELLO_INO, &inode_hello); // 13 is HELLO_INO
	...
	write_inode(fd, HELLO_WORLD_INO, &hello_world_inode); // 12 is HELLO_WORLD_INO
}
...
void write_root_dir_block(int fd)
{
	...
	ssize_t bytes_left = BLOCK_SIZE;

	struct ext2_dir_entry current_entry = {0};
	dir_entry_set(current_entry, EXT2_ROOT_INO, "."); // 2 is root inode #
	dir_entry_write(current_entry, fd);

	bytes_left -= current_entry.rec_len;

	struct ext2_dir_entry parent_entry = {0};
	dir_entry_set(parent_entry, EXT2_ROOT_INO, ".."); // 2 is root inode #
	dir_entry_write(parent_entry, fd);

	bytes_left -= parent_entry.rec_len;

	struct ext2_dir_entry hello = {0};
	dir_entry_set(hello, HELLO_INO, "hello"); // 13 is hello inode #
	dir_entry_write(hello, fd);

	bytes_left -= hello.rec_len;

	struct ext2_dir_entry hello_world = {0};
	dir_entry_set(hello_world, HELLO_WORLD_INO, "hello-world"); // 12 is hello-world inode #
	dir_entry_write(hello_world, fd);

	bytes_left -= hello_world.rec_len;

	struct ext2_dir_entry lost = {0};
	dir_entry_set(lost, LOST_AND_FOUND_INO, "lost+found"); // 11 is lost and found inode #
	dir_entry_write(lost, fd);

	bytes_left -= lost.rec_len;

	struct ext2_dir_entry fill_entry = {0};
	fill_entry.rec_len = bytes_left;
...
void write_hello_world_file_block(int fd)
{
	...
	// printf(HELLO_WORLD_FILE_BLOCKNO);
}
